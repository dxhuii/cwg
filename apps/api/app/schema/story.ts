...
import { BaseModel, BaseModelStatic, IStory } from '../typings'
export interface StoryType extends BaseModel, IStory {}

export default (app: Context & Application) => {
  const { model, Sequelize } = app
  // 获取数据类型
  const { INTEGER, DATE, NOW, BOOLEAN, STRING, TEXT, SMALLINT } = Sequelize

  return model.define('story', {
