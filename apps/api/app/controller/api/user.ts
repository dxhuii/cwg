...
export default class User extends Controller {
  ...
  async add() {
    ...
    // 校验参数
    const { username, email, captcha, id, password, token, nickname } = params
  ...
  public async setAdmin() {
    const { ctx, service } = this
    const { id } = ctx.query
    const data = await service.user.findUser({ admin: 100 })
    if (!data) {
      const result = await service.user.edit({ id, admin: 100 })
      if (result) {
